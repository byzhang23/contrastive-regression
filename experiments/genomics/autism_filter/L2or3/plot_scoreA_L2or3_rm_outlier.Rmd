---
title: "plot_scoreA_L2or3_rm_outliers"
...
```{r read in data}
ct <-  'L2or3' # cell type
score <- 'scoreA_rm_outlier' # score

output_dir <- paste0('./',score,'/')
outliers <- c('5976_BA9','5978_BA9','6033_BA9')


score <- readRDS(paste0('../scoreA_pfc.rds')) %>% filter(!(sample %in% c(outliers)))
...
ggplot(contrastive.long %>% filter(gene %in% annot.genes), aes(x = zscore_new, y = expr, color = diagnosis)) +
  ...
  scale_color_manual(breaks = c('ASD','Control'),
                     values = c('#AF46B4','#4BB446')) +
  labs(x = 'Z score', y = 'Contrastive expression')
ggsave(paste0(output_dir, 'contrastive_scatter_annotated.png'), height = 4, width = 7)
```
...
(ggpairs(z_auxiliary, columns = 6:ncol(z_auxiliary),
